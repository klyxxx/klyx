...
function locationMatches(
  ...
  return [provider.city, ...provider.serviceArea].some(
    (location) => {
...
function budgetResult(
  ...
  return {
    fits:
      budget == null ||
      (Number.isFinite(budget) && estimated <= budget),
    estimated,
    budget:
...
export function explainProviderMatch(
  ...
  if (budget.fits) {
    ...
    if (
      ...
      reasons.push("Tarif clairement renseigné");
    }
  } else if (
    budget.estimated != null &&
    budget.budget != null
    ...
    warnings.push("Tarif encore à confirmer");
  }

  const trustPoints = Math.round(
    Math.max(0, Math.min(100, provider.klyxScore)) *
      0.2
  );
  score += trustPoints;

  if (provider.klyxScore >= 80) {
    reasons.push("Très bon score de confiance");
  } else if (provider.klyxScore >= 60) {
    reasons.push("Score de confiance correct");
  } else {
    warnings.push(
  ...
  if (provider.cancellationRate <= 0.1) {
    score += 5;
    reasons.push("Faible taux d’annulation");
  ...
  if (
    filters.pricing !== "all" &&
    provider.pricingType === filters.pricing
  ) {
    score += 5;
    reasons.push("Type de tarif souhaité");
  }
