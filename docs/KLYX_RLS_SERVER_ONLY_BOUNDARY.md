...
The post-booking incident tables and the single/group settlement control planes added after that live audit inherit the same server-only boundary: RLS is enabled and browser roles are revoked. Settlement tables are writable only through trusted server/service-role code and audited RPC boundaries.
