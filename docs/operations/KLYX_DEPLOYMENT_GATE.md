...
A successful `push` run is the normal proof. If a push run was cancelled before useful execution, a later successful `workflow_dispatch` run is an accepted recovery proof **only when its `head_sha` equals the current exact `main` SHA already resolved by the deployment gate**. A branch label or manual invocation never substitutes for the SHA equality.
