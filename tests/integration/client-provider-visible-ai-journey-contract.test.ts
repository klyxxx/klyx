...
describe("KLYX visible AI journeys", () => {
  ...
  it("uses one visible OpenAI wording call without changing deterministic facts", () => {
    ...
    const guidedQuestion = read("lib/brain/guided-question.ts");
    ...
    expect(clientRoute).toContain("requiredTail");
    expect(clientRoute).toContain("suppressAiQuestions: Boolean(requiredTail)");
    ...
    expect(visibleAi).toContain("withoutQuestions");
    expect(visibleAi).toContain("requiredTail");
    expect(visibleAi).toContain("KLYX ajoutera lui-même");
    expect(visibleAi).toContain('mode: "fallback"');

    expect(guidedQuestion).toContain("KLYX_CONFIRMATION_BOUNDARY");
    expect(guidedQuestion).toContain("De quel service as-tu besoin ?");
    expect(guidedQuestion).toContain("Dans quelle ville ou commune");
    expect(guidedQuestion).toContain("Quel jour souhaites-tu");
    expect(guidedQuestion).toContain("À quel moment souhaites-tu");
  });
});
