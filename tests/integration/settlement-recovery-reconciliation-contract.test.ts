...
describe("KLYX settlement recovery / reconciliation contract", () => {
  ...
  it("certifies Transfer-created / DB-not-finalized recovery through the real KLYX reconciliation engine", () => {
    expect(stripeNetworkProof).toContain(
    ...
    expect(stripeNetworkProof).toContain(
      '"network_recovery_post_transfer_restriction"'
    );
    expect(stripeNetworkProof).toContain(
      "transferMatchesAfterRecovery.length === 1"
    );
    expect(stripeNetworkProof).not.toContain(
