...
$sendDir = Join-Path $projectRoot "app\api\profile\phone\otp\send"
$verifyDir = Join-Path $projectRoot "app\api\profile\phone\otp\verify"

$sendPath = Join-Path $sendDir "route.ts"
$verifyPath = Join-Path $verifyDir "route.ts"

$phonePath = Join-Path $projectRoot "app\settings\PhoneSettingsInline.tsx"

Write-Host ""
Write-Host "KLYX 12.69 - Phone OTP Verification"
Write-Host ""

if (-not (Test-Path -LiteralPath $phonePath)) {
    throw "PhoneSettingsInline.tsx introuvable."
}
...
$helperContent = [string]::Join("`n", $helperLines)
...
$sendContent = [string]::Join("`n", $sendLines)
...
$verifyContent = [string]::Join("`n", $verifyLines)

# ============================================================
# 4. PHONE SETTINGS COMPLETE REPLACEMENT
# ============================================================
...
$phoneContent = [string]::Join("`n", $phoneLines)
...
$checks = @(
    ...
    @{
        Name = "OTP UI"
        Value = $phoneContent.Contains(
            "KLYX_PHONE_OTP_UI_12_69"
        )
    },
    @{
        Name = "phone verified update"
        Value = $verifyContent.Contains(
            "phone_verified_at: verifiedAt"
        )
    },
...
foreach ($check in $checks) {
    if (-not $check.Value) {
        throw "12.69 validation failed : $($check.Name)"
    }
}
...
Write-Host ""
Write-Host "======================================"
Write-Host "KLYX 12.69 APPLIQUE"
Write-Host "======================================"
Write-Host "SMS OTP server endpoints crees."
Write-Host "UI verification telephone ajoutee."
Write-Host "phone_verified_at protege cote serveur."
Write-Host ""