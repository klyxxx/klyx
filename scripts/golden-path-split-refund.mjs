...
async function postStripeEvent(event, webhookSecret) {
  const payload = JSON.stringify(event);
  const timestamp = Math.floor(Date.now() / 1000);
  const response = await fetch(`${APP_ORIGIN}/api/stripe/webhook`, {
    method: "POST",
    headers: {
      "content-type": "application/json",
      "stripe-signature": stripeSignature(payload, webhookSecret, timestamp),
    },
    body: payload,
  });
...
async function insertOne(admin, table, payload, columns = "*") {
...
async function loadOne(admin, table, id, columns = "*") {
  const { data, error } = await admin
    .from(table)
    .select(columns)
    .eq("id", id)
...
async function loadBookings(admin, bookingIds) {
  const { data, error } = await admin
    .from("bookings")
    .select(
  ...
  if (error) {
    throw new Error(`Unable to load split child bookings: ${error.message}`);
  }

  expect(
    (data ?? []).length === bookingIds.length,
...
async function loadLedger(admin, bookingIds, entryType) {
  const { data, error } = await admin
  ...
  if (error) throw new Error(`Unable to load financial ledger: ${error.message}`);
  return data ?? [];
}

async function assertWebhookHandled(event, webhookSecret) {
  const body = await postStripeEvent(event, webhookSecret);
  expect(body?.received === true, `Webhook ${event.type} was not acknowledged.`);
  expect(
  ...
  expect(body?.eventId === event.id, `Webhook ${event.type} returned the wrong event id.`);
  return body;
}
...
async function linkGoldenCanonicalStripeAccount(
  admin,
  authUserId,
  stripeAccountId
) {
...
async function main() {
  ...
  expect(Boolean(user), "Golden-path auth user must exist before split refund proof.");

  const { data: profiles, error: profilesError } = await admin
    .from("profiles")
    .select("id, owner_user_id, account_type, country_code, currency_code")
  ...
  const providerOne = (profiles ?? []).find(
    (profile) => profile.account_type === "provider"
  );

  expect(
    Boolean(client) && Boolean(providerOne),
  ...
  if (servicesError) {
    throw new Error(`Unable to load KLYX services: ${servicesError.message}`);
  }

  const service = cleaningService(services ?? []);
  expect(Boolean(service), "Canonical cleaning service is missing from split refund proof.");

  const { data: providerOneService, error: providerOneServiceError } = await admin
    .from("user_services")
    .select("id")
    .eq("user_id", providerOne.id)
  ...
  /*
   * Both legacy provider profiles belong to the same canonical KLYX account.
   * Financial identity is therefore account-level: both payment units must use
   * one canonical Connected Account. Keeping two synthetic Stripe accounts here
  ...
  await insertOne(
    ...
    "profiles",
    {
      ...
      country_code: "BE",
      currency_code: "EUR",
      stripe_account_id: canonicalStripeAccountId,
      stripe_onboarding_complete: true,
      stripe_charges_enabled: true,
      stripe_payouts_enabled: true,
    },
  ...
  const { error: providerTwoAvailabilityError } = await admin
    .from("availability_slots")
    .insert(
      Array.from({ length: 7 }, (_, dayOfWeek) => ({
        user_service_id: providerTwoService.id,
  ...
  const marketRequest = await insertOne(
    admin,
    "market_service_requests",
    {
      client_profile_id: client.id,
      service_id: service.id,
      title: "Golden split refund reconciliation",
      description: "Ephemeral KLYX split payment and refund reconciliation proof.",
      city: "Bruxelles",
  ...
  const priceHash = "b".repeat(64);
  const paymentPlanHash = "c".repeat(64);
  ...
  const planConfirmation = await insertOne(
    ...
    "market_split_plan_confirmations",
    {
      ...
      client_profile_id: client.id,
      plan_hash: planHash,
      plan_snapshot: { golden: true, slots: 3, providers: 2 },
      slot_count: 3,
      provider_count: 2,
    },
    "id"
  ...
  const bookingSpecs = [
    {
      ...
      userServiceId: providerOneService.id,
      amount: 4000,
      start: "09:00",
      end: "10:00",
      position: 1,
      slotId: `golden-split-${nonce}-1`,
    },
    {
      providerId: providerOne.id,
      userServiceId: providerOneService.id,
      ...
      position: 2,
      slotId: `golden-split-${nonce}-2`,
    },
    {
      providerId: providerTwoId,
      userServiceId: providerTwoService.id,
      amount: 5000,
      start: "13:00",
      end: "14:00",
      position: 3,
      slotId: `golden-split-${nonce}-3`,
    },
  ];
  ...
  for (const spec of bookingSpecs) {
    const booking = await insertOne(
      ...
      "bookings",
      {
        ...
        end_time: spec.end,
        status: "accepted",
        message: "Golden split refund proof",
        payment_status: "unpaid",
        amount_total: spec.amount,
        estimated_amount_cents: spec.amount,
        currency: "EUR",
        service_id: service.id,
        user_service_id: spec.userServiceId,
  ...
  const { error: batchItemsError } = await admin
    ...
    .insert(
      bookings.map((booking) => ({
        ...
        user_service_id: booking.userServiceId,
      }))
    );

  if (batchItemsError) {
    throw new Error(`Unable to create split batch items: ${batchItemsError.message}`);
  }

  const { error: batchReadyError } = await admin
    .from("split_booking_batches")
    .update({ status: "created", completed_at: now, updated_at: now })
    .eq("id", batch.id);

  if (batchReadyError) {
    throw new Error(`Unable to finalize split batch: ${batchReadyError.message}`);
  }
  ...
  const priceConfirmation = await insertOne(
    ...
    "split_booking_price_confirmations",
    {
      ...
      price_snapshot: { golden: true, total_amount_cents: 15000 },
      item_count: 3,
  ...
  const paymentConfirmation = await insertOne(
    ...
    "split_booking_payment_confirmations",
    {
      ...
      payment_plan_hash: paymentPlanHash,
  ...
  const unitOne = await insertOne(
    ...
    "split_booking_payment_units",
    {
      ...
      slot_ids: [bookings[0].slotId, bookings[1].slotId],
      application_fee_amount: 1500,
      provider_amount_cents: 8500,
      status: "pending",
    },
    "id"
  );

  const unitTwo = await insertOne(
    admin,
    "split_booking_payment_units",
    {
      run_id: run.id,
      ...
      payment_confirmation_id: paymentConfirmation.id,
      client_profile_id: client.id,
      provider_profile_id: providerTwoId,
      stripe_account_id: canonicalStripeAccountId,
      amount_cents: 5000,
      currency: "EUR",
      booking_ids: [bookings[2].id],
      slot_ids: [bookings[2].slotId],
      application_fee_amount: 750,
      provider_amount_cents: 4250,
      status: "pending",
    },
    "id"
  );

  const sessionOneId = `cs_test_split_one_${nonce}`;
  const sessionTwoId = `cs_test_split_two_${nonce}`;
  const intentOneId = `pi_test_split_one_${nonce}`;
  const intentTwoId = `pi_test_split_two_${nonce}`;

  await assertWebhookHandled(
    stripeEvent(`evt_test_split_pay_one_${nonce}`, "checkout.session.completed", {
      id: sessionOneId,
      object: "checkout.session",
      amount_total: 10000,
      currency: "eur",
      payment_status: "paid",
      payment_intent: intentOneId,
      metadata: {
        klyx_flow: "split_payment_13_27",
        split_payment_unit_id: unitOne.id,
      },
    }),
    webhookSecret
  );

  let storedRun = await loadOne(
    admin,
    "split_booking_payment_runs",
    run.id,
    "id, status"
  ...
  expect(
    storedRun.status === "partially_paid",
    `Split run should be partially_paid after first unit, got ${storedRun.status}.`
  );

  await assertWebhookHandled(
    stripeEvent(`evt_test_split_pay_two_${nonce}`, "checkout.session.completed", {
      id: sessionTwoId,
      object: "checkout.session",
      amount_total: 5000,
      currency: "eur",
      payment_status: "paid",
      payment_intent: intentTwoId,
      metadata: {
        klyx_flow: "split_payment_13_27",
        split_payment_unit_id: unitTwo.id,
      },
    }),
    webhookSecret
  );

  storedRun = await loadOne(
    admin,
    "split_booking_payment_runs",
    run.id,
    "id, status"
  ...
  let bookingMap = await loadBookings(admin, bookingIds);

  for (const booking of bookings) {
    const stored = bookingMap.get(booking.id);
    expect(stored?.payment_status === "paid", `Split child ${booking.id} should be paid.`);
    expect(
      stored?.payment_mode === "connect_destination_split",
      `Split child ${booking.id} should preserve split payment mode.`
    );
  }

  expect(
    bookingMap.get(bookings[0].id)?.stripe_checkout_session_id === sessionOneId &&
      bookingMap.get(bookings[1].id)?.stripe_checkout_session_id === sessionOneId,
    "Two child bookings must be allowed to share the unit-level Stripe Checkout Session."
  );

  const paymentLedger = await loadLedger(admin, bookingIds, "payment_succeeded");
  expect(
  ...
  expect(
    paymentLedger.every(
      (entry) =>
        entry.payment_mode === "connect_destination_split" &&
  ...
  expect(
    paymentLedger.reduce(
      (total, entry) => total + Number(entry.platform_fee_cents ?? 0),
      0
    ) === 2250,
    "Split payment ledger platform fee total must equal 2250 cents."
  );

  const partialRefundId = `re_test_split_partial_${nonce}`;
  const partialRefundEvent = stripeEvent(
    ...
    "refund.created",
    {
      ...
      status: "succeeded",
      failure_reason: null,
      metadata: { split_payment_unit_id: unitOne.id },
    }
  );

  await assertWebhookHandled(partialRefundEvent, webhookSecret);

  let storedUnitOne = await loadOne(
    admin,
    "split_booking_payment_units",
    unitOne.id,
  ...
  expect(
    storedUnitOne.refund_status === "partially_refunded" &&
      Number(storedUnitOne.refunded_amount_cents) === 2500,
  ...
  expect(
    storedRun.status === "partially_refunded",
    "Split run must become partially_refunded after the first partial refund."
  );

  bookingMap = await loadBookings(admin, bookingIds);
  expect(
    Number(bookingMap.get(bookings[0].id)?.refunded_amount_cents) === 1000 &&
      Number(bookingMap.get(bookings[1].id)?.refunded_amount_cents) === 1500,
    "Partial split refund must allocate 1000/1500 cents across 40/60 child bookings."
  );
  expect(
    bookingMap.get(bookings[0].id)?.payment_status === "paid" &&
      bookingMap.get(bookings[1].id)?.payment_status === "paid",
    "Partially refunded child bookings must remain payment_status=paid."
  );
  expect(
    bookingMap.get(bookings[0].id)?.stripe_refund_id === partialRefundId &&
      bookingMap.get(bookings[1].id)?.stripe_refund_id === partialRefundId,
    "Split children must be allowed to share the unit-level Stripe refund id."
  );

  const refundLedgerBeforeReplay = await loadLedger(
    admin,
    bookingIds,
    "refund_succeeded"
  );
  expect(
    refundLedgerBeforeReplay.length === 2,
    `Expected 2 partial refund ledger rows before replay, got ${refundLedgerBeforeReplay.length}.`
  );

  const duplicateBody = await postStripeEvent(partialRefundEvent, webhookSecret);
  expect(
    duplicateBody?.received === true &&
      duplicateBody?.duplicate === true &&
      duplicateBody?.reason === "already_processed" &&
      duplicateBody?.eventId === partialRefundEvent.id,
    "Exact split refund webhook replay must be rejected as an already_processed duplicate."
  );

  const refundLedgerAfterReplay = await loadLedger(
    admin,
    bookingIds,
    "refund_succeeded"
  );
  expect(
    refundLedgerAfterReplay.length === refundLedgerBeforeReplay.length,
    "Duplicate split refund webhook must not create extra ledger rows."
  );

  const { count: partialRefundCount, error: partialRefundCountError } = await admin
    .from("split_booking_payment_refunds")
    .select("id", { count: "exact", head: true })
    .eq("stripe_refund_id", partialRefundId);
  ...
  const finalUnitOneRefundId = `re_test_split_final_one_${nonce}`;
  await assertWebhookHandled(
    stripeEvent(`evt_test_split_refund_final_one_${nonce}`, "refund.created", {
  ...
  expect(
    storedUnitOne.refund_status === "refunded" &&
      Number(storedUnitOne.refunded_amount_cents) === 10000,
    "First split unit must become fully refunded at 10000 cents."
  );

  bookingMap = await loadBookings(admin, bookingIds);
  expect(
    Number(bookingMap.get(bookings[0].id)?.refunded_amount_cents) === 4000 &&
      Number(bookingMap.get(bookings[1].id)?.refunded_amount_cents) === 6000,
    "Cumulative split refund snapshots must reach the exact 4000/6000 child totals."
  );
  expect(
    bookingMap.get(bookings[0].id)?.payment_status === "refunded" &&
      bookingMap.get(bookings[1].id)?.payment_status === "refunded",
    "Fully refunded split children must reach terminal payment_status=refunded."
  );

  const unitTwoRefundId = `re_test_split_final_two_${nonce}`;
  await assertWebhookHandled(
    stripeEvent(`evt_test_split_refund_final_two_${nonce}`, "refund.created", {
      id: unitTwoRefundId,
      object: "refund",
      amount: 5000,
      currency: "eur",
      ...
      status: "succeeded",
      failure_reason: null,
      metadata: { split_payment_unit_id: unitTwo.id },
    }),
    webhookSecret
  );

  const storedUnitTwo = await loadOne(
    admin,
    "split_booking_payment_units",
    unitTwo.id,
    "id, refund_status, refunded_amount_cents"
  ...
  expect(
    storedUnitTwo.refund_status === "refunded" &&
      Number(storedUnitTwo.refunded_amount_cents) === 5000,
  ...
  expect(
    bookings.every(
      (booking) => bookingMap.get(booking.id)?.payment_status === "refunded"
    ),
    "Every split child booking must end in terminal payment_status=refunded."
  );
  expect(
    Number(bookingMap.get(bookings[2].id)?.refunded_amount_cents) === 5000,
    "Second split unit child refund snapshot must equal 5000 cents."
  );

  const finalRefundLedger = await loadLedger(admin, bookingIds, "refund_succeeded");
  expect(
    finalRefundLedger.length === 5,
    `Expected 5 split refund ledger rows, got ${finalRefundLedger.length}.`
  );

  const refundedByBooking = new Map(bookingIds.map((id) => [id, 0]));
  for (const entry of finalRefundLedger) {
    expect(
      entry.payment_mode === "connect_destination_split" &&
        entry.status === "succeeded",
    ...
    refundedByBooking.set(
      entry.booking_id,
      (refundedByBooking.get(entry.booking_id) ?? 0) +
        Number(entry.refund_amount_cents ?? 0)
    );
  }

  expect(
    refundedByBooking.get(bookings[0].id) === 4000 &&
      refundedByBooking.get(bookings[1].id) === 6000 &&
      refundedByBooking.get(bookings[2].id) === 5000,
  ...
  expect(
    (refundRows ?? []).length === 3 &&
      (refundRows ?? []).every((refund) => refund.status === "succeeded") &&
      (refundRows ?? []).reduce(
        (total, refund) => total + Number(refund.amount_cents),
        0
      ) === 15000,
  ...
  process.stdout.write(
    `${JSON.stringify({
      verified: true,
      flow: "split_payment_13_27",
      runId: run.id,
      paymentUnitCount: 2,
      childBookingCount: 3,
      paymentLedgerRows: paymentLedger.length,
      refundRows: (refundRows ?? []).length,
      refundLedgerRows: finalRefundLedger.length,
      duplicateWebhookRejected: true,
      canonicalStripeAccountSharedAcrossLegacyProfiles: true,
      totalPaidCents: 15000,
      totalRefundedCents: 15000,
      finalRunStatus: storedRun.status,
    })}\n`
  );
}
...
main().catch((error) => {
  ...
  console.error(`KLYX golden-path split refund failed: ${message}`);
  process.exitCode = 1;
});