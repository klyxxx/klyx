...
$checks = @(
    ...
    @{
        Name = "remainingCount expose"
        Value = $content.Contains(
            "remainingCount: remainingCompletionCount,"
        )
    },
    @{
        Name = "statusText expose"
        Value = $content.Contains(
            "statusText: completionStatusText,"
        )
    },
    @{
        Name = "guided question conservee"
        Value = $content.Contains(
            "question: nextCompletionQuestion,"
        )
    },
    @{
        Name = "completion score conserve"
        Value = $content.Contains(
...
if ($failed) {
    throw "KLYX 12.52 checker FAILED."
}
...
Write-Host ""
Write-Host "======================================"
Write-Host "KLYX 12.52 CHECK OK"
Write-Host "Brain Progress Feedback operationnel."
Write-Host "======================================"
Write-Host ""