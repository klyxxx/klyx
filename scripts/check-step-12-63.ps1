...
if (-not $pagePath) {
    throw "Page assistant KLYX 12.63 introuvable."
}
...
$checks = @(
    ...
    @{
        Name = "12.63 page marker"
        Value = $page.Contains(
            "KLYX_EXPLICIT_CONFIRMATION_12_63"
        )
    },
    @{
        Name = "12.63 API marker"
        Value = $api.Contains(
    ...
    @{
        Name = "complete request required"
        Value = $pageCompact.Contains(
    ...
    @{
        Name = "existing review flow preserved"
        Value = $pageCompact.Contains(
            "openResults();"
        )
    },
...
if ($failed.Count -gt 0) {
    ...
    throw "KLYX 12.63 static checker FAILED."
}
