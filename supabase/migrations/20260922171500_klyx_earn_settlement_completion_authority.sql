-- KLYX MISSION 19 — EARN SETTLEMENT COMPLETION AUTHORITY
--
-- The workflow state machine is orchestration state, never financial authority.
-- An earn workflow may become terminal only after canonical single-booking
-- settlement truth proves that the provider was actually released.
...
begin
  ...
  begin
    v_booking_id := nullif(trim(coalesce(p_payload ->> 'booking_id', '')), '')::uuid;
  exception
  ...
  if not found
     or coalesce(v_booking.provider_id, v_booking.babysitter_id) is distinct from v_workflow.profile_id
...
comment on function public.klyx_complete_settlement_workflow(
  ...
  'Mission 19: completes earn workflow only from canonical released booking settlement truth for the same provider.';
