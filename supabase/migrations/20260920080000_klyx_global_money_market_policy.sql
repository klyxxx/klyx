...
begin
  if not exists (
    ...
    alter table public.bookings
      ...
      check (
        payer_country_code is null
        or payer_country_code ~ '^[A-Z]{2}$'
      );
  end if;
  ...
  if not exists (
    ...
    alter table public.bookings
      ...
      check (
        execution_country_code is null
        or execution_country_code ~ '^[A-Z]{2}$'
      );
  end if;
  ...
  if not exists (
    ...
    alter table public.bookings
      ...
      check (
        ...
        and (
          tax_mode is null
          or tax_mode in ('none', 'fixed_rate', 'stripe_tax', 'external')
        )
        and (
          tax_liability is null
          or tax_liability in ('provider', 'platform', 'stripe')
        )
      );
  end if;
  ...
  if not exists (
    select 1 from pg_constraint
    where conname = 'bookings_market_payment_rule_fk'
  ) then
    alter table public.bookings
      add constraint bookings_market_payment_rule_fk
      foreign key (market_payment_rule_id)
      references public.klyx_market_payment_rules(id)
      on delete restrict;
  end if;

  if not exists (
    select 1 from pg_constraint
    where conname = 'bookings_fx_quote_fk'
  ) then
    alter table public.bookings
      add constraint bookings_fx_quote_fk
      foreign key (fx_quote_id)
      references public.klyx_fx_quotes(id)
      on delete restrict;
  end if;
end
$$;
...
begin
  if not exists (
    select 1 from pg_constraint
    where conname = 'booking_financial_ledger_market_rule_fk'
  ) then
    alter table public.booking_financial_ledger
      add constraint booking_financial_ledger_market_rule_fk
      foreign key (market_payment_rule_id)
      references public.klyx_market_payment_rules(id)
      on delete restrict;
  end if;
  ...
  if not exists (
    select 1 from pg_constraint
    where conname = 'booking_financial_ledger_fx_quote_fk'
  ) then
    alter table public.booking_financial_ledger
      add constraint booking_financial_ledger_fx_quote_fk
      foreign key (fx_quote_id)
      references public.klyx_fx_quotes(id)
      on delete restrict;
  end if;
...
comment on table public.klyx_payment_currency_capabilities is
  'Reviewed payment-provider currency capabilities by execution country and currency; refreshed as provider support evolves.';
comment on table public.klyx_fx_quotes is
  'Client-bound immutable FX quote truth used for explicit, non-silent currency conversion.';
