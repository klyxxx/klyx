...
function parsePriceItems(value: unknown): PriceItem[] {
  ...
  for (const raw of root.items) {
    ...
    const amount = numeric(item.amountCents);
...
function parsePaymentPlan(value: unknown): PaymentPlan | null {
  const root = asRecord(value);
  if (!root || !Array.isArray(root.units)) {
    return null;
  }

  const batchId = text(root.batchId);
  const priceConfirmationId = text(root.priceConfirmationId);
  const providerCount = numeric(root.providerCount);
  const paymentUnitCount = numeric(root.paymentUnitCount);
  const totalAmountCents = numeric(root.totalAmountCents);
  ...
  for (const raw of root.units) {
    ...
    const amountCents = numeric(unit.amountCents);
    const unitCurrency = text(unit.currency).toUpperCase();
    const stripeAccountId = text(unit.stripeAccountId);
    const bookingIds = stringArray(unit.bookingIds).sort();
    const slotIds = stringArray(unit.slotIds).sort();
  ...
  return {
    batchId,
    priceConfirmationId,
    providerCount: Math.round(providerCount),
    paymentUnitCount: Math.round(paymentUnitCount),
...
function hashPlan(plan: PaymentPlan): string {
  return createHash("sha256").update(JSON.stringify(plan)).digest("hex");
}

function bookingAmount(booking: JsonRow): number | null {
  const estimated = numeric(booking.estimated_amount_cents);
  if (estimated !== null && estimated >= 0) {
    return Math.round(estimated);
  }

  const total = numeric(booking.amount_total);
...
function paymentAlreadyClaimed(booking: JsonRow): boolean {
...
async function getUnits(runId: string): Promise<UnitRow[]> {
  const { data, error } = await supabaseAdmin
...
function publicUnits(units: UnitRow[]) {
  return units.map((unit) => ({
    id: unit.id,
    providerId: unit.provider_profile_id,
    amountCents: Number(unit.amount_cents),
    currency: unit.currency,
    bookingCount: stringArray(unit.booking_ids).length,
    status: unit.status,
    checkoutUrl: unit.status === "checkout_open" ? unit.checkout_url : null,
    paid: unit.status === "paid",
  }));
}

function findPlanUnit(plan: PaymentPlan, providerId: string): PaymentUnitPlan {
  const planUnit = plan.units.find(
    (candidate) => candidate.providerId === providerId
  );
  if (!planUnit) {
    throw new Error("Unité canonique introuvable.");
  }
  return planUnit;
}
...
async function createCheckoutSession(params: {
  ...
  const metadata = {
    ...
    split_payment_run_id: runId,
    split_payment_unit_id: unit.id,
  ...
  return stripe.checkout.sessions.create(
    {
      ...
      success_url:
        origin + "/bookings/split/" + batchId + "?split_payment=success",
      cancel_url:
        origin + "/bookings/split/" + batchId + "?split_payment=cancelled",
      line_items: [
        {
          quantity: 1,
          price_data: {
            currency: planUnit.currency.toLowerCase(),
            unit_amount: planUnit.amountCents,
            product_data: {
              name: "Mission KLYX · paiement prestataire",
            },
          },
        },
      ],
      metadata,
      payment_intent_data: {
        application_fee_amount: economics.platformFeeCents,
        transfer_data: {
          destination: planUnit.stripeAccountId,
        },
        metadata,
      },
    },
    {
      idempotencyKey:
        "klyx-split-unit-" + unit.id + "-attempt-" + attemptNumber,
    }
  );
}

async function attachCheckout(params: {
  unitId: string;
  attemptToken: string;
  session: Stripe.Checkout.Session;
}) {
  const { unitId, attemptToken, session } = params;
  if (!session.url) {
    throw new Error("Stripe n'a pas retourné de lien Checkout.");
  }

  const { data: attached, error } = await supabaseAdmin.rpc(
  ...
  if (attached !== true) {
    throw new Error("Le verrou Checkout split a changé.");
  }
}
...
export async function GET(request: Request, context: RouteContext) {
  ...
  try {
    ...
    const run = await getExistingRun(batchId);
    ...
    if (!run) {
      return NextResponse.json({
        ...
        moneyMovedAutomatically: false,
      });
    }

    if (run.client_profile_id !== profile.id) {
      return NextResponse.json({ error: "Accès refusé." }, { status: 403 });
    }

    const units = await getUnits(run.id);
    return NextResponse.json({
      prepared: true,
      runId: run.id,
      status: run.status,
      totalAmountCents: Number(run.total_amount_cents),
      currency: run.currency,
      paymentUnitCount: run.payment_unit_count,
      paidUnitCount: units.filter((unit) => unit.status === "paid").length,
    ...
    return secureApiErrorResponse({
      error,
      event: "split_checkout_read_failed",
      route: "/api/bookings/split-missions/[id]/checkout",
      method: "GET",
      code: "split_checkout_read_failed",
      status: 500,
      startedAt,
    });
  }
...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
      ...
      return NextResponse.json(
        {
          ...
          code: "SPLIT_CHECKOUT_MARKET_NOT_READY",
          participant: "client",
    ...
    const { id: batchId } = await context.params;
    let run = await getExistingRun(batchId);
    ...
    if (run) {
      ...
      const { data, error } = await supabaseAdmin
        ...
        .eq("id", run.payment_confirmation_id)
        .maybeSingle();
      if (error) {
        throw new Error(error.message);
      }
      confirmation = data as unknown as ConfirmationRow | null;
    } else {
      const { data, error } = await supabaseAdmin
        .from("split_booking_payment_confirmations")
        .select(
    ...
    if (!confirmation || confirmation.invalidated_at) {
      return NextResponse.json(
        {
          error: "La confirmation finale du paiement est absente ou invalide.",
          code: "SPLIT_PAYMENT_CONFIRMATION_REQUIRED",
        },
        { status: 409 }
      );
    }

    const plan = parsePaymentPlan(confirmation.payment_plan_snapshot);
    if (
      !plan ||
      ...
      return NextResponse.json(
        { error: "Plan de paiement invalide.", code: "SPLIT_PAYMENT_PLAN_INVALID" },
        { status: 409 }
      );
    }

    const computedHash = hashPlan(plan);
    if (computedHash !== confirmation.payment_plan_hash) {
      return NextResponse.json(
        {
          error: "La preuve du plan de paiement ne correspond plus.",
          code: "SPLIT_PAYMENT_PLAN_HASH_MISMATCH",
        },
        { status: 409 }
      );
    }

    if (run && run.payment_plan_hash !== computedHash) {
      return NextResponse.json(
        {
          error: "Un autre plan de paiement a déjà commencé pour cette mission.",
          code: "SPLIT_PAYMENT_RUN_LOCKED",
        },
        { status: 409 }
      );
    }
    ...
    const { data: priceData, error: priceError } = await supabaseAdmin
      ...
      .select("id, batch_id, price_snapshot, invalidated_at")
      .eq("id", plan.priceConfirmationId)
      .maybeSingle();
    if (priceError) {
      throw new Error(priceError.message);
    }

    const priceProof = priceData as unknown as PriceProofRow | null;
    if (
      !priceProof ||
      priceProof.invalidated_at ||
      priceProof.batch_id !== batchId
    ) {
      return NextResponse.json(
        { error: "La preuve de prix n'est plus valide.", code: "SPLIT_PRICE_PROOF_INVALID" },
        { status: 409 }
      );
    }

    const priceItems = parsePriceItems(priceProof.price_snapshot);
    const allBookingIds = priceItems.map((item) => item.bookingId);
    ...
    const bookings = (bookingData ?? []) as unknown as JsonRow[];
    const bookingById = new Map<string, JsonRow>();
    ...
    if (bookings.length !== priceItems.length) {
      return NextResponse.json(
        {
          error: "Les réservations de la mission ont changé.",
          code: "SPLIT_BOOKING_STRUCTURE_CHANGED",
        },
        { status: 409 }
      );
    }
    ...
    for (const item of priceItems) {
      ...
      if (
        !booking ||
        text(booking.parent_id) !== profile.id ||
        !bookingAccepted(booking) ||
        bookingAmount(booking) !== item.amountCents ||
        text(booking.currency).toUpperCase() !== item.currency
      ) {
        return NextResponse.json(
          {
            error: "Une réservation ne correspond plus à la confirmation.",
            code: "SPLIT_LIVE_BOOKING_CHANGED",
          },
          { status: 409 }
        );
      }

      if (!run && paymentAlreadyClaimed(booking)) {
        return NextResponse.json(
          {
            error: "Une réservation possède déjà un paiement ou un Checkout.",
            code: "SPLIT_CHILD_ALREADY_HAS_PAYMENT",
          },
          { status: 409 }
        );
      }
    ...
    const providerById = new Map<string, JsonRow>();
    for (const provider of (providerData ?? []) as unknown as JsonRow[]) {
    ...
    for (const unit of plan.units) {
      const provider = providerById.get(unit.providerId);
      if (!provider) {
        return NextResponse.json(
          { error: "Prestataire introuvable.", code: "SPLIT_PROVIDER_NOT_FOUND" },
          { status: 409 }
        );
      }
      ...
      const account = await stripe.accounts.retrieve(liveAccountId);
      const deleted =
        (account as unknown as { deleted?: boolean }).deleted === true;
      if (deleted) {
        return NextResponse.json(
          {
            error: "Un prestataire n'est plus prêt à recevoir un paiement Stripe.",
            code: "SPLIT_PROVIDER_STRIPE_NOT_READY",
          },
          { status: 409 }
        );
      }
      ...
      const countryAssessment = assessStripeConnectCountry({
        klyxCountryCode: text(provider.country_code),
      ...
      if (
        liveAccount.charges_enabled !== true ||
        liveAccount.payouts_enabled !== true ||
        liveAccount.details_submitted !== true ||
        (liveAccount.requirements?.currently_due?.length ?? 0) > 0
      ) {
        return NextResponse.json(
          {
            error: "Un prestataire n'est plus prêt à recevoir un paiement Stripe.",
    ...
    if (!run) {
      const { data: insertedRun, error: runError } = await supabaseAdmin
        ...
        .insert({
          ...
          payment_plan_hash: computedHash,
          total_amount_cents: plan.totalAmountCents,
          currency: plan.currency,
          provider_count: plan.providerCount,
          payment_unit_count: plan.paymentUnitCount,
          status: "preparing",
        })
        .select(
          "id, batch_id, client_profile_id, payment_confirmation_id, payment_plan_hash, total_amount_cents, currency, provider_count, payment_unit_count, status"
      ...
      run = insertedRun as unknown as RunRow | null;
      if (!run) {
        run = await getExistingRun(batchId);
      }
      if (!run) {
        throw new Error("Impossible de verrouiller le run de paiement split.");
      }
    }
        ...
        .upsert(
          {
            ...
            payment_confirmation_id: confirmation.id,
            client_profile_id: profile.id,
            provider_profile_id: unit.providerId,
            stripe_account_id: unit.stripeAccountId,
            amount_cents: unit.amountCents,
            currency: unit.currency,
            booking_ids: unit.bookingIds,
            slot_ids: unit.slotIds,
            application_fee_amount: economics.platformFeeCents,
            provider_amount_cents: economics.providerAmountCents,
          },
          {
            onConflict: "run_id,provider_profile_id",
    ...
    let units = await getUnits(run.id);
    if (units.length !== plan.paymentUnitCount) {
      throw new Error("Nombre d'unités de paiement incohérent.");
    }
    ...
    for (const unit of units) {
      ...
      const { data: claimData, error: claimError } = await supabaseAdmin.rpc(
        ...
        {
          p_unit_id: unit.id,
          p_client_profile_id: profile.id,
          p_attempt_token: attemptToken,
        }
      );
      if (claimError) {
      ...
      let claim = ((claimData ?? []) as unknown as ClaimRow[])[0];
      if (!claim) {
        throw new Error("Claim de paiement split absent.");
      }
      if (claim.action === "busy") {
        return NextResponse.json(
          {
            error: "Une autre préparation de paiement est déjà en cours.",
            paymentPending: true,
          },
          { status: 409 }
        );
      ...
      if (claim.action === "reuse" && claim.checkout_session_id) {
        ...
        if (existing.status === "expired") {
          ...
          claim = ((retryData ?? []) as unknown as ClaimRow[])[0];
          if (!claim || claim.action !== "create") {
            throw new Error("Impossible de renouveler le Checkout expiré.");
          }

          const planUnit = findPlanUnit(plan, unit.provider_profile_id);
      ...
      const { error: bookingLockError } = await supabaseAdmin
        .from("bookings")
        .update({
          payment_status: "checkout_created",
          payment_mode: "connect_destination_split",
          stripe_checkout_session_id: session.id,
          payment_attempt_token: null,
          payment_checkout_started_at: null,
        })
        .in("id", planUnit.bookingIds)
        .neq("payment_status", "paid");
      if (bookingLockError) {
        throw new Error(bookingLockError.message);
      }
    }
    ...
    if (finalized !== true) {
      throw new Error("Toutes les unités Checkout ne sont pas encore prêtes.");
    }

    units = await getUnits(run.id);
    return NextResponse.json({
      prepared: true,
      runId: run.id,
      status: units.every((unit) => unit.status === "paid")
        ? "paid"
        : units.some((unit) => unit.status === "paid")
          ? "partially_paid"
          : "ready",
      totalAmountCents: plan.totalAmountCents,
      currency: plan.currency,
      paymentUnitCount: plan.paymentUnitCount,
      units: publicUnits(units),
      proofConsumed: true,
      explicitStripeCheckoutRequired: true,
    ...
    return secureApiErrorResponse({
      error,
      event: "split_checkout_prepare_failed",
      route: "/api/bookings/split-missions/[id]/checkout",
      method: "POST",
      code: "split_checkout_prepare_failed",
      status,
