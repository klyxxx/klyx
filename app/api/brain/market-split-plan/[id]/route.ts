...
import { buildLiveMultiProviderSplitPlan } from "@/lib/multi-provider-split-plan";
  ...
  try {
    ...
    if (!profile.canRequestServices) {
      return NextResponse.json(
        { error: "Cette analyse nécessite la capacité de demander un service." },
        { status: 403 }
      );
    }
    ...
    const marketRequest = data ? (data as unknown as RequestRow) : null;
    ...
    if (marketRequest.client_profile_id !== profile.id) {
      return NextResponse.json(
        { error: "Cette demande ne t appartient pas." },
        { status: 403 }
      );
    }

    if (marketRequest.request_mode !== "multi_slot") {
      return NextResponse.json(
        {
          error:
            "Le plan multi-prestataires concerne uniquement les missions multi-creneaux.",
          code: "SPLIT_PLAN_MULTI_SLOT_REQUIRED",
        },
        { status: 409 }
      );
    }
    ...
    if (marketRequest.status !== "open") {
      return NextResponse.json(
        {
          error: "Cette demande n est plus ouverte.",
          code: "SPLIT_PLAN_REQUEST_NOT_OPEN",
        },
        { status: 409 }
      );
    }

    const plan = await buildLiveMultiProviderSplitPlan({
      requestId: marketRequest.id,
      serviceId: marketRequest.service_id,
      expectedSlotCount: Number(marketRequest.slot_count),
    });

    let message = "";

    if (plan.mode === "single_provider_available") {
      message =
        "Un prestataire couvre encore tous les creneaux. KLYX le privilegie avant toute repartition.";
    } else if (plan.mode === "split_available") {
      message =
        "Aucun prestataire unique ne couvre toute la mission, mais KLYX a trouve une combinaison de " +
        String(plan.providerCount) +
        " prestataires couvrant tous les creneaux.";
    } else {
      message =
        plan.uncoveredPositions.length > 0
    ...
    return NextResponse.json({
      request: {
        ...
        slotCount: Number(marketRequest.slot_count),
