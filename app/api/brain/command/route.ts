...
export async function POST(request: Request) {
  try {
    ...
    // IMPORTANT 12.81:
    // Actions are recalculated from DB server-side and this route only routes.
    const actions = (
      await getBrainActions(profile)
    ).slice(0, 20) as BrainActionItem[];

    const specificExistingIntent =
      hasSpecificBrainCommandIntent(message);
    const generalActionIntent =
      hasGeneralBrainCommandIntent(message);
    const newNeedIntent =
      hasNewNeedBrainCommandIntent(message);

    if (
      actions.length > 0 &&
      (specificExistingIntent || generalActionIntent)
    ) {
      const action = bestBrainCommandAction(actions, message);
