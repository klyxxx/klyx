...
export default function FavoritesPage() {
  ...
  if (loading) {
    return <main className="grid min-h-screen place-items-center bg-background dark:bg-zinc-950 text-foreground dark:text-white"><p className="text-sm text-muted-foreground dark:text-zinc-400">Chargement des favoris...</p></main>;
  }

  return (
    <main className="min-h-screen overflow-x-hidden bg-background dark:bg-zinc-950 px-3 py-5 text-foreground dark:text-white sm:px-6 sm:py-8 lg:px-8">
      <div className="mx-auto min-w-0 max-w-7xl">
        <Link href="/search" className="inline-flex items-center gap-2 text-sm font-bold text-muted-foreground dark:text-zinc-400 transition hover:text-foreground dark:text-white"><ArrowLeft size={17} />Retour à la recherche</Link>
        ...
        {favorites.length === 0 ? (
          <section className="mt-8 rounded-3xl border border-border dark:border-zinc-800 bg-card/60 dark:bg-zinc-900/60 p-7 text-center sm:p-10">
          ...
          <div className="mt-8 grid min-w-0 gap-5 sm:grid-cols-2 xl:grid-cols-3">
            {favorites.map((favorite) => (
              <article key={favorite.serviceProfileId} className="min-w-0 overflow-hidden rounded-3xl border border-border dark:border-zinc-800 bg-card dark:bg-zinc-900">
                <div className="relative aspect-[16/10] overflow-hidden bg-muted dark:bg-zinc-800">
                  {favorite.avatarUrl ? <img src={favorite.avatarUrl} alt={favorite.fullName} className="h-full w-full object-cover" /> : <div className="grid h-full place-items-center text-center text-muted-foreground dark:text-zinc-500"><div><Heart className="mx-auto" size={30} /><p className="mt-2 text-xs font-bold">Prestataire KLYX</p></div></div>}
