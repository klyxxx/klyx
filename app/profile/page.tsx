...
export default function ProfilePage() {
  ...
  const loadProfile = useCallback(async () => {
    ...
    try {
      ...
      if (!data) throw new Error("Profil KLYX introuvable.");
  ...
  async function uploadAvatar(event: ChangeEvent<HTMLInputElement>) {
    ...
    if (!file.type.startsWith("image/") || file.size > 5 * 1024 * 1024) {
      setErrorMessage("Choisis une image JPG, PNG ou WEBP de 5 Mo maximum.");
      return;
    }
  ...
  async function saveProfile(event: FormEvent<HTMLFormElement>) {
    ...
    if (!firstName.trim() || !lastName.trim() || !city.trim()) {
      setErrorMessage("Le prénom, le nom et la ville sont obligatoires.");
      return;
    }

    if (
      normalizedAge !== null &&
      (!Number.isInteger(normalizedAge) || normalizedAge < 18 || normalizedAge > 100)
    ) {
      setErrorMessage("L’âge doit être compris entre 18 et 100 ans.");
      return;
    }
    ...
    return (
      <main className="flex min-h-screen items-center justify-center bg-zinc-950 text-white">
        <div className="text-center">
          <LoaderCircle className="mx-auto animate-spin text-violet-400" size={40} />
    ...
    <main className="min-h-screen bg-zinc-950 px-4 py-8 text-white sm:px-6">
      <div className="mx-auto max-w-4xl">
        ...
        <header className="mt-8">
          ...
          <h1 className="mt-3 text-3xl font-bold sm:text-5xl">Mes informations</h1>
        ...
        >
          <div className="flex flex-col gap-6 border-b border-zinc-800 pb-8 sm:flex-row sm:items-center">
            <div className="flex h-36 w-36 shrink-0 items-center justify-center overflow-hidden rounded-full border-4 border-violet-600 bg-zinc-800">
              {avatarUrl ? (
                <img src={avatarUrl} alt={fullName || "Photo de profil"} className="h-full w-full object-cover" />
              ) : (
          ...
          >
            {saving ? <LoaderCircle className="animate-spin" size={19} /> : <Save size={19} />}
