  ...
  async function createPlan(
    ...
    try {
      ...
      if (!response.ok || !body.plan) {
        throw new Error(
          body.error || "Création impossible."
        );
      }
      ...
      <div className="mx-auto max-w-6xl">
        <section className="relative overflow-hidden rounded-[2rem] border border-white/10 bg-[linear-gradient(135deg,#17131f,#3c1764_52%,#101827)] p-7 text-white sm:p-10">
          ...
          <p className="mt-4 max-w-3xl text-sm leading-7 text-white/70">
            Décris ton objectif. KLYX prépare le plan, la recherche
            et les prochaines actions, mais ne réserve et ne paie
            jamais sans ta confirmation.
          </p>
        </section>
          ...
          <div className="mt-5 grid gap-3 md:grid-cols-2">
            {EXAMPLES.map((example) => (
              <button
                ...
                disabled={loading}
                onClick={() =>
                  void createPlan(undefined, example)
                }
        ...
        <div className="mt-8 flex gap-3 rounded-2xl border border-emerald-500/25 bg-emerald-500/10 p-5">
          ...
          <p className="text-sm leading-6 text-muted-foreground">
            Chaque étape importante exige ton action. KLYX ne choisit
            pas le prestataire, ne confirme pas la réservation et ne
            déclenche jamais le paiement automatiquement.
          </p>
        </div>
      </div>
    </main>
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        ...
        {!["completed", "cancelled"].includes(
          ...
          <button
            type="button"
            disabled={
              busyId === `${plan.id}:cancel`
            }
            onClick={() =>
              void onProcess(plan.id, "cancel")
            }
